...
def lidar_data(point_cloud, lidar_point_cloud_buffer):
    matrix_representational_data = np.copy(np.frombuffer(point_cloud.raw data, dtype=np.dtype("f4")))
    reshape_of_image = np.reshape(matrix_representational_data, (int(matrix_representational_data.shape[0] / 4), 4))
    intensity = reshape of image[:,-1]
    lidar_points = reshape of imagel[:,:-1]
    lidar_points[:, :1] = -lidar points[:, :1]
    lidar_point_cloud_buffer['pts'] = lidar_points
    lidar_point_cloud_buffer['intensity'] = intensity


def generate _lidar_blueprint (blueprint library):
lidar_blueprint = blueprint_library.find('sensor.lidar.ray_cast')
lidar_blueprint.set_attribute('channels', str(32))
lidar_blueprint.set_attribute('points per second',str(50000))
lidar_blueprint.set_attribute('rotation frequency',str(50))
